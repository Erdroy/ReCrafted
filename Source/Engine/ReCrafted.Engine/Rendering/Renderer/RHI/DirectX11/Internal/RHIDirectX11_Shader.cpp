        ...
        for (auto& il : m_inputLayouts)
        {
            if (il.elements.size() == inputLayoutDesc.size())
            {
                for (auto i = 0u; i < inputLayoutDesc.size(); i++)
                {
                    ...
                    auto& current = inputLayoutDesc[i];
    ...
    RHIDirectX11_Shader* RHIDirectX11_Shader::Create(ID3D11Device* device, nlohmann::json& shaderJson)
    {
        ...
        for (auto& bufferJson : buffers)
        {
            ...
            for (auto& fieldData : bufferUniforms)
            {
                ...
                const auto length = fieldData["Length"].get<int>();
        ...
        auto samplers = shaderJson["SamplerStates"];
        for (auto& sampler : samplers)
        {
            auto samplerName = sampler.get<std::string>();
