...
void Profiler::DrawThreadProfiles(ThreadData* thread)
{
    ...
    if (ImGui::CollapsingHeader(thread->threadName))
    {
        ...
        for (auto i = 0u; i < thread->frames.size(); i++)
        {
            const auto frameTime = thread->frames[i].time_ms;
        ...
        const auto hoveredFrame = ImGui::PlotLines("", frameTimes, NumProfiledFrames, 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 128));
        ...
        const auto graphSize = ImGui::GetItemRectSize();
        const auto startPos = ImGui::GetItemRectMin();
        ...
        if (thread->selectedFrame >= 0)
        {
            ...
            {
                // Select frame
                const auto& selectedFrame = thread->frames[thread->selectedFrame];

                // Compile profiles
                List<ProfileTreeEntry> treeProfiles;
                ...
                for (const auto& event : treeProfiles)
                {
...
{
    const auto open = ImGui::TreeNodeEx(event.name.c_str(), event.children.Empty() ? ImGuiTreeNodeFlags_Leaf : 0);
    ...
    if (open)
    {
        ...
        for (auto&& childEvent : event.children)
        {
