...
static inline float vec4_dot(const float x[4], const float y[4])
{
    return x[0] * y[0] + x[1] * y[1] + x[2] * y[2] + x[3] * y[3];
}

static inline float vec4_length2(const float x[4])
{
    return vec4_dot(x, x);
}
    ...
    LinearBuffer<vec4>& collapseNormal)
{
    ...
    for (int i : randomEdges)
    {
        ...
        vec4_set(collapsePosition[i], pos);
