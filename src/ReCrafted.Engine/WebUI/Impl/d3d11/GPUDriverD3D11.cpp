...
{
    GPUDriverD3D11::GPUDriverD3D11(PlatformGPUContextD3D11* context) : context_(context)
    {
    }
                                            ...
                                            const RenderBuffer& buffer)
    {
        ...
        {
            MessageBoxW(nullptr,
                        L"GPUDriverD3D11::CreateRenderBuffer, render buffer ID 0 is reserved for default render target view.",
                        L"Error", MB_OK);
            return;
        }
                                          ...
                                          ID3D11InputLayout** ppInputLayout)
    {
        ...
        {
            MessageBoxW(nullptr,
                        L"GPUDriverD3D11::LoadVertexShader, Vertex shader could not be compiled. Check your working directory.",
                        L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D11::LoadPixelShader(const WCHAR* path, ID3D11PixelShader** ppPixelShader)
    {
        ...
        {
            MessageBoxW(nullptr,
                        L"GPUDriverD3D11::LoadPixelShader, Pixel shader could not be compiled. Check your working directory.",
                        L"Error", MB_OK);
        }
    }
    ...
    void GPUDriverD3D11::BindGeometry(uint32_t id)
    {
        ...
        UINT stride = geometry.format == kVertexBufferFormat_2f_4ub_2f
                          ? sizeof(Vertex_2f_4ub_2f)
    ...
    ComPtr<ID3D11SamplerState> GPUDriverD3D11::GetSamplerState()
    {
        ...
        HRESULT hr = context_->device()->CreateSamplerState(&sampler_desc, sampler_state_.GetAddressOf());
