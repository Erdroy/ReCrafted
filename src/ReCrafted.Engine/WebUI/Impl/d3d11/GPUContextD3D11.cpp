...
public:
  ...
  virtual ID3D11RenderTargetView* render_target_view() override { 
...
std::unique_ptr<GPUContext> GPUContext::Create(bool enable_vsync) {
  ...
  if (gpu_context->Initialize(static_cast<HWND>(Platform::GetCurrentWindow()), Display::GetWidth(), Display::GetHeight(), 1.0f, false, enable_vsync, false, 1))
