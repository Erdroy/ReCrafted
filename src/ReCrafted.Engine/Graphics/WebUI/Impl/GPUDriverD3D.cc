...
namespace ultralight {
    ...
    GPUDriverD3D::GPUDriverD3D(D3DContext* context) : context_(context) {
    }

    GPUDriverD3D::~GPUDriverD3D() {
    }

    void GPUDriverD3D::BeginSynchronize() {
    }

    void GPUDriverD3D::EndSynchronize() {
    }

    uint32_t GPUDriverD3D::NextTextureId() { return next_texture_id_++; }

    void GPUDriverD3D::CreateTexture(uint32_t texture_id,
        Ref<Bitmap> bitmap) {
        auto i = textures_.find(texture_id);
        if (i != textures_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateTexture, texture id already exists.", L"Error", MB_OK);
            return;
        }

        if (bitmap->format() != kBitmapFormat_RGBA8) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateTexture, unsupported format.", L"Error", MB_OK);
        }
        ...
        if (FAILED(hr)) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateTexture, unable to create texture.", L"Error", MB_OK);
        }
    ...
    void GPUDriverD3D::UpdateTexture(uint32_t texture_id,
        Ref<Bitmap> bitmap) {
        auto i = textures_.find(texture_id);
        if (i == textures_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::UpdateTexture, texture id doesn't exist.", L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D::BindTexture(uint8_t texture_unit,
        uint32_t texture_id) {
        auto i = textures_.find(texture_id);
        if (i == textures_.end()) {
            //MessageBox(nullptr,
            //  L"GPUDriverD3D::BindTexture, texture id doesn't exist.", L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D::DestroyTexture(uint32_t texture_id) {
        auto i = textures_.find(texture_id);
        if (i != textures_.end()) {
    ...
    uint32_t GPUDriverD3D::NextRenderBufferId() { return next_render_buffer_id_++; }

    void GPUDriverD3D::CreateRenderBuffer(uint32_t render_buffer_id,
        const RenderBuffer& buffer) {
        if (render_buffer_id == 0) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateRenderBuffer, render buffer ID 0 is reserved for default render target view.", L"Error", MB_OK);
            return;
        }
        ...
        if (i != render_targets_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateRenderBuffer, render buffer id already exists.", L"Error", MB_OK);
            return;
        }
        ...
        if (tex_entry == textures_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateRenderBuffer, texture id doesn't exist.", L"Error", MB_OK);
            return;
        }
        ...
        if (FAILED(hr)) {
            MessageBox(nullptr,
                L"GPUDriverD3D::CreateRenderBuffer, unable to create render target.", L"Error", MB_OK);
        }
    }

    void GPUDriverD3D::BindRenderBuffer(uint32_t render_buffer_id) {
        ...
        if (i == render_targets_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::BindRenderBuffer, render buffer id doesn't exist.", L"Error", MB_OK);
            return;
        }

        context_->immediate_context()->OMSetRenderTargets(1, i->second.GetAddressOf(), nullptr);
    }

    void GPUDriverD3D::ClearRenderBuffer(uint32_t render_buffer_id) {
        ...
        if (i == render_targets_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::ClearRenderBuffer, render buffer id doesn't exist.", L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D::DestroyRenderBuffer(uint32_t render_buffer_id) {
        auto i = render_targets_.find(render_buffer_id);
        if (i != render_targets_.end()) {
            i->second.Reset();
    ...
    uint32_t GPUDriverD3D::NextGeometryId() { return next_geometry_id_++; }

    void GPUDriverD3D::CreateGeometry(uint32_t geometry_id,
    ...
    void GPUDriverD3D::UpdateGeometry(uint32_t geometry_id,
        const VertexBuffer& vertices,
        const IndexBuffer& indices) {
        auto i = geometry_.find(geometry_id);
        if (i == geometry_.end()) {
            MessageBox(nullptr,
                L"GPUDriverD3D::UpdateGeometry, geometry id doesn't exist.", L"Error", MB_OK);
            return;
        }
    ...
    void GPUDriverD3D::DrawGeometry(uint32_t geometry_id,
    ...
    void GPUDriverD3D::DestroyGeometry(uint32_t geometry_id) {
        auto i = geometry_.find(geometry_id);
        if (i != geometry_.end()) {
            i->second.first.Reset();
            i->second.second.Reset();
    ...
    void GPUDriverD3D::UpdateCommandList(const CommandList& list) {
        if (list.size) {
    ...
    void GPUDriverD3D::DrawCommandList() {
    ...
    ComPtr<ID3D11InputLayout> GPUDriverD3D::GetVertexLayout() {
        if (vertex_layout_)
    ...
    ComPtr<ID3D11SamplerState> GPUDriverD3D::GetSamplerState() {
        if (sampler_state_)
        ...
        HRESULT hr = context_->device()->CreateSamplerState(&sampler_desc, &ss);
        sampler_state_ = ss;

        if (FAILED(hr)) {
            MessageBox(nullptr,
                L"GPUDriverD3D::GetSamplerState, unable to create sampler state.", L"Error", MB_OK);
        }

        return sampler_state_;
    }

    ComPtr<ID3D11Buffer> GPUDriverD3D::GetConstantBuffer() {
        if (constant_buffer_)
        ...
        if (FAILED(hr)) {
            MessageBox(nullptr,
                L"GPUDriverD3D::GetConstantBuffer, unable to create constant buffer.", L"Error", MB_OK);
        }

        return constant_buffer_;
    }

    void GPUDriverD3D::UpdateConstantBuffer(const GPUState& state) {
