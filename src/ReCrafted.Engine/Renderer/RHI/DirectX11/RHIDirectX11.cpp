...
#pragma region WorkerThread command execution implementation
        ...
        void WorkerThreadInstance::Execute_CreateTexture2D(Command_CreateTexture2D* command)
        {
            ...
            if(command->memory)
            {
                cvar dxgiFormat = DGXI_TextureFormats[command->textureFormat][0];
                ...
                if(command->generateMips)
                {
                    ...
                    m_context->UpdateSubresource(texture.texture, 0, nullptr, memory, rowPitch, 0);
                }
                else
                {
                    // Upload all mips
                    for (var mipId = 0; mipId < command->mipLevels; mipId++)
                    {
                        ...
                        // Update mip
                        m_context->UpdateSubresource(texture.texture, mipId, nullptr, memory, rowPitch, 0);
...
#pragma region RHI interface implementation
        ...
        void RHIDirectX11::CopyTextureSubresource(Texture2DHandle targetTexture, void* sourceTexturePtr, uint8_t subresourceId)
        {
            ...
            m_deviceContext->CopyResource(textureDesc.texture, static_cast<ID3D11Texture2D*>(sourceTexturePtr));
